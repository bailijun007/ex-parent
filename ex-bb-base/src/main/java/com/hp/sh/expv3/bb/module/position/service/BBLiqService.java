...
import com.hp.sh.expv3.bb.constant.TimeInForce;
import com.hp.sh.expv3.bb.job.LiqHandleResult;
import com.hp.sh.expv3.bb.module.order.service.BBOrderService;
import com.hp.sh.expv3.bb.module.position.dao.BBLiqRecordDAO;
...
import com.hp.sh.expv3.bb.mq.liq.msg.CancelOrder;
...
public class BBLiqService {
	private static final Logger logger = LoggerFactory.getLogger(BBLiqService.class);
    ...
    @Autowired
    private BBPositionMarginService bBPositionMarginService;
    ...
    @Autowired
    private BBLiqRecordDAO bBLiqRecordDAO;
	...
	public LiqHandleResult checkPosLiq(PosUID pos) {
    	BBPosition bBPosition = this.positionDataService.getPosition(pos.getUserId(), pos.getId());
		LiqHandleResult liqResult = new LiqHandleResult();
		
		MarkPriceVo markPriceVo = markPriceService.getLastMarkPrice(bBPosition.getAsset(), bBPosition.getSymbol());
		//检查触发强平
		if(!this.checkAndResetLiqStatus(bBPosition, markPriceVo.getMarkPrice())){
			return liqResult;
		}
		
		//追加保证金
		if(bBPosition.getAutoAddFlag()==IntBool.YES){
			this.bBPositionMarginService.autoAddMargin(bBPosition);
		}
		...
		//强平仓位,修改状态等
		this.lockLiq(bBPosition);
		
		liqResult.setTrigger(true);
		liqResult.setMarkPriceVo(markPriceVo);
		liqResult.setLiqPrice(bBPosition.getLiqPrice());
		liqResult.setPcPosition(bBPosition);
		return liqResult;
	 ...
	 * @return 是否触发强平
	 */
	private boolean checkAndResetLiqStatus(BBPosition pos, BigDecimal markPrice) {
	 ...
	 * @return 是否触发强平
	 */
	public boolean checkLiqStatus(BBPosition pos, BigDecimal markPrice) {
	...
	private void lockLiq(BBPosition pos) {
	...
	public void cancelCloseOrder(Long userId, String asset, String symbol, Integer longFlag, Long posId, List<CancelOrder> list, Integer lastFlag) {
		BBPosition pos = this.positionDataService.getPosition(userId, asset, symbol, posId);
		...
		for(CancelOrder co : list){
			this.bBOrderService.cance4Liq(userId, asset, symbol, co.getOrderId(), co.getCancelNumber());
		}
    ...
    @LockIt(key="${pos.userId}-${pos.asset}-${pos.symbol}")
	public void forceClose(BBPosition pos) {
    	this.doLiq(pos);
    }
	
	private void doLiq(BBPosition pos){
		Long now = DbDateUtils.now();
		//1、保存强平记录
		BBLiqRecord record = this.saveLiqRecord(pos, now);
		//2、清空仓位
...
//		this.createLiqOrder(record);
	...
	private void clearLiqPos(BBPosition pos, Long now){
	...
	private BBLiqRecord saveLiqRecord(BBPosition pos, Long now){
		BBLiqRecord record = new BBLiqRecord();
		...
		//save
		bBLiqRecordDAO.save(record);
		publisher.publishEvent(record);
		return record;
	}
	
	private void createLiqOrder(BBLiqRecord record){
		BBPosition pos = positionDataService.getPosition(record.getUserId(), record.getAsset(), record.getSymbol(), record.getPosId());
		this.bBOrderService.create(record.getUserId(), "LIQ-"+record.getId(), record.getAsset(), record.getSymbol(), OrderFlag.ACTION_CLOSE, record.getLongFlag(), TimeInForce.IMMEDIATE_OR_CANCEL, record.getBankruptPrice(), record.getVolume(), pos, IntBool.NO, IntBool.YES);
	}
