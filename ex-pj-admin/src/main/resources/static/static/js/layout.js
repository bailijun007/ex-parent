...
function checkAll(chkobj) {
    if ($(chkobj).text() == "全选") {
        ...
        $(".checkReply").prop("checked", true);
    } else {
        $(chkobj).children("span").text("全选");
        $(".checkall input:enabled").prop("checked", false);
        $(".checkReply").prop("checked", false);
    }
}
//Tab控制函数
...
function ExePostBack(objId, objmsg) {
    if ($(".checkall input:checked").size() < 1 && $(".ask input:checkbox:checked").size() < 1) {
...
function CustomConFirm(objId, objmsg,param1,param2) {

    var msg = "删除记录后不可恢复，您确定吗？";
    if (arguments.length == 4) {
        msg = objmsg;
        if ($("#" + param1 + "").val() == "" || $("#" + param1 + "").val().length<2) {
            $("#spanName").attr("class", " Validform_wrong");
            $("#spanName").text("请填写信息！");
          
            return false;
        }
        else if ($("#" + param2 + "").val() == "" || $("#" + param2 + "").val().length<2) {
            $("#spanUnion").attr("class", " Validform_wrong");
            $("#spanUnion").text("请填写信息！");
            return false;
        }
        else {
            msg = "会员名称:<span style='color:red'>" + $("#" + param1 + "").val() + "</span><br/>工会名称:<span style='color:red'>" + $("#" + param2 + "").val() + "</span><br/>"+msg;
        }
...
function PassCustomConFirm(objId, objmsg, param1, param2,TypeDesc) {

    var msg = "删除记录后不可恢复，您确定吗？";
    if (arguments.length == 5) {
        msg = objmsg;
        if ($("#" + param1 + "").val() == "" || $("#" + param1 + "").val().length < 1) {
            $("#spanType").show();
            $("#spanType").attr("class", " Validform_wrong");
            $("#spanType").text("请填写信息！");
            return false;
        }
        else if ($("#" + param2 + "").val() == "" || $("#" + param2 + "").val().length < 1) {
            $("#spanRemark").show();
            $("#spanRemark").attr("class", " Validform_wrong");
            $("#spanRemark").text("请填写信息！");
            return false;
        }
        else {
            msg = "不通过类型:<span style='color:red'>" + $("#" + param1 + "").val() + "</span><br/>审核意见:<span style='color:red'>" + $("#" + param2 + "").val() + "</span><br/>" + msg + $("#" + TypeDesc + "").val();
        }
        $("#spanType").hide();
        $("#spanRemark").hide();
    }
...
function VoteDetailConFirm(objId, objmsg, Title) {
    var msg = "删除记录后不可恢复，您确定吗？";
    if (objmsg.length > 3)
        msg = objmsg;
    msg += "<br/><span style='line-height:20px;'>投票项：<span style='color:red'>" + $("#" + Title + "").val() + "</span></span>";
    $.dialog.confirm(msg, function () {
...
$.fn.ruleMultiPorp = function() {
	var multiPorp = function(parentObj){
		...
		parentObj.find(":checkbox").each(function(){
			...
			var newObj = $('<a href="javascript:;" data-value="'+$(this).prop("value")+'">' + parentObj.find('label').eq(indexNum).text() + '</a><i></i>').appendTo(liObj); //查找对应Label创建选项
			if($(this).prop("checked") == true){
...
$.fn.ruleMultiRadio = function() {
	var multiRadio = function(parentObj){
		...
		parentObj.find('input[type="radio"]').each(function(){
			...
			var newObj = $('<a href="javascript:;" data-value="' + parentObj.find('input[type="radio"]').eq(indexNum).val() + '">' + parentObj.find('label').eq(indexNum).text() + '</a>').appendTo(divObj); //查找对应Label创建选项
			if($(this).prop("checked") == true){
				newObj.addClass("selected"); //默认选中
			}
			//检查控件是否启用
			...
			$(newObj).click(function(){
				...
				parentObj.find('input[type="radio"]').eq(indexNum).attr("checked","checked");
