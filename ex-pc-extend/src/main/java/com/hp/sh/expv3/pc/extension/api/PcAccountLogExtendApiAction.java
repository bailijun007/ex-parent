...
public class PcAccountLogExtendApiAction implements PcAccountLogExtendApi {
    ...
    public PageResult<PcAccountRecordLogVo> query(Long userId, String asset, Integer tradeType, Integer historyType, Long startDate, Long endDate, Integer pageNo, Integer pageSize, String symbol) {
        ...
        if (!CollectionUtils.isEmpty(pcAccountLogList.getList())) {
            ...
            if (type2Refs.containsKey(PcAccountLog.TYPE_LIQ_LONG) || type2Refs.containsKey(PcAccountLog.TYPE_LIQ_SHORT)) {
...
//            for (PcAccountLogVo pcAccountLogVo : pcAccountLogList.getList()) {
//                PcAccountRecordLogVo recordLogVo = null;
...
//            }
        ...
        //如果查询全部需要重新进行分页截取
//        rePageCut(tradeType, pageNo, pageSize, result, list);
     ...
     * @param list
     */
    private void rePageCut(Integer tradeType, Integer pageNo, Integer pageSize, PageResult<PcAccountRecordLogVo> result, List<PcAccountRecordLogVo> list) {
    ...
    private void appendOrderTradeData(BigDecimal faceValue, String asset, String symbol, Long userId, List<PcAccountRecordLogVo> recordLogVos) {
    ...
    private void appendTransferMarginData(BigDecimal faceValue, String asset, String symbol, Long userId, List<PcAccountRecordLogVo> recordLogVos) {
        List<Long> refIds = recordLogVos.stream().map(PcAccountRecordLogVo::getRefId).collect(Collectors.toList());
        ...
        if (null != pcLiqRecordVo) {
            final Map<Long, PcAccountRecordVo> id2Vo = pcLiqRecordVo.stream().collect(Collectors.toMap(PcAccountRecordVo::getId, Function.identity()));
            for (PcAccountRecordLogVo recordLogVo : recordLogVos) {
     ...
     */
    private void appendPcLiqRecordData(BigDecimal faceValue, String asset, String symbol, Long userId, List<PcAccountRecordLogVo> recordLogVos) {
        List<Long> refIds = recordLogVos.stream().map(PcAccountRecordLogVo::getRefId).collect(Collectors.toList());
        List<PcLiqRecordVo> pcLiqRecordVo = pcLiqRecordService.listPcLiqRecord(asset, symbol, userId, refIds);
        if (null != pcLiqRecordVo) {
            final Map<Long, PcLiqRecordVo> id2Vo = pcLiqRecordVo.stream().collect(Collectors.toMap(PcLiqRecordVo::getId, Function.identity()));
            for (PcAccountRecordLogVo recordLogVo : recordLogVos) {
