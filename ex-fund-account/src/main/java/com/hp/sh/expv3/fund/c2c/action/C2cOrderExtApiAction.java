    ...
    public String withdrawalOrder(Long userId, String bankCard, String bank, String bankCardName, String srcAsset, BigDecimal srcNum, String tarAsset, BigDecimal tarNum, BigDecimal ratio) {
        ...
        if (remain.subtract(srcNum).compareTo(BigDecimal.ZERO) >= 0) {
            try {
                ...
                c2cOrder.setPayTime(timestamp);
                c2cOrder.setPayFinishTime(timestamp);
                ...
                c2cOrder.setCreated(timestamp);
                c2cOrder.setModified(timestamp);
                c2cOrder.setVolume(srcNum);
                c2cOrder.setAmount(ratio.multiply(srcNum));
                c2cOrder.setFrozenAsset(srcNum);
